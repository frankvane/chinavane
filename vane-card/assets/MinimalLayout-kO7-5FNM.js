const n=`/**\r
 * 极简布局示例\r
 * 只有图片和标题的极简设计，适合分类导航\r
 */\r
\r
import {\r
  CardCore,\r
  createHoverPlugin,\r
  createImagePlugin,\r
  withPlugins,\r
} from "vane-card";\r
\r
import DemoPage from "../_layout/DemoPage";\r
import React from "react";\r
\r
interface CategoryData {\r
  id: string;\r
  name: string;\r
  image: string;\r
  count?: number;\r
}\r
\r
const MinimalCard = withPlugins(CardCore, {\r
  plugins: [\r
    createImagePlugin({ lazyLoad: true }),\r
    createHoverPlugin({ enableScale: true }),\r
  ],\r
});\r
\r
export default function MinimalLayout() {\r
  const categories: CategoryData[] = [\r
    {\r
      id: "1",\r
      name: "耳机音响",\r
      image:\r
        "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=400",\r
      count: 234,\r
    },\r
    {\r
      id: "2",\r
      name: "智能穿戴",\r
      image:\r
        "https://images.unsplash.com/photo-1575311373937-040b8e1fd5b6?w=400",\r
      count: 156,\r
    },\r
    {\r
      id: "3",\r
      name: "充电配件",\r
      image:\r
        "https://images.unsplash.com/photo-1583863788434-e58a36330cf0?w=400",\r
      count: 89,\r
    },\r
    {\r
      id: "4",\r
      name: "蓝牙音箱",\r
      image:\r
        "https://images.unsplash.com/photo-1608043152269-423dbba4e7e1?w=400",\r
      count: 78,\r
    },\r
    {\r
      id: "5",\r
      name: "智能手表",\r
      image:\r
        "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=400",\r
      count: 123,\r
    },\r
    {\r
      id: "6",\r
      name: "电脑外设",\r
      image:\r
        "https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=400",\r
      count: 201,\r
    },\r
  ];\r
\r
  return (\r
    <DemoPage\r
      title="极简布局"\r
      description="只有图片和标题的极简设计，适合分类导航、图标墙"\r
    >\r
      <div\r
        style={{\r
          display: "grid",\r
          gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))",\r
          gap: "16px",\r
        }}\r
      >\r
        {categories.map((category) => (\r
          <MinimalCard\r
            key={category.id}\r
            cardId={category.id}\r
            data={category}\r
            containerStyle={{\r
              background: "#fff",\r
              borderRadius: "8px",\r
              overflow: "hidden",\r
              cursor: "pointer",\r
            }}\r
          >\r
            <div\r
              style={{\r
                padding: "12px",\r
                textAlign: "center",\r
              }}\r
            >\r
              <h4\r
                style={{\r
                  margin: "0 0 4px 0",\r
                  fontSize: "14px",\r
                  fontWeight: "500",\r
                  color: "#333",\r
                }}\r
              >\r
                {category.name}\r
              </h4>\r
              {category.count && (\r
                <p\r
                  style={{\r
                    margin: 0,\r
                    fontSize: "12px",\r
                    color: "#999",\r
                  }}\r
                >\r
                  {category.count} 件商品\r
                </p>\r
              )}\r
            </div>\r
          </MinimalCard>\r
        ))}\r
      </div>\r
\r
      {/* 说明文档 */}\r
      <div\r
        style={{\r
          marginTop: "40px",\r
          padding: "20px",\r
          background: "#f5f5f5",\r
          borderRadius: "8px",\r
        }}\r
      >\r
        <h2 style={{ marginTop: 0 }}>布局说明</h2>\r
        <h3>适用场景</h3>\r
        <ul style={{ lineHeight: 1.8 }}>\r
          <li>商品分类导航</li>\r
          <li>品牌墙展示</li>\r
          <li>专题入口</li>\r
          <li>快速选择界面</li>\r
        </ul>\r
\r
        <h3>布局特点</h3>\r
        <ul style={{ lineHeight: 1.8 }}>\r
          <li>信息极简，只保留核心内容</li>\r
          <li>图片为主，文字为辅</li>\r
          <li>悬停时有缩放效果，增强交互</li>\r
          <li>适合第一级导航或入口展示</li>\r
        </ul>\r
\r
        <h3>设计要点</h3>\r
        <ul style={{ lineHeight: 1.8 }}>\r
          <li>图片建议使用清晰、代表性强的素材</li>\r
          <li>标题简短，通常 2-6 个字</li>\r
          <li>可选添加副标题（如商品数量）</li>\r
          <li>整体布局干净，降低认知负担</li>\r
        </ul>\r
\r
        <h3>交互建议</h3>\r
        <ul style={{ lineHeight: 1.8 }}>\r
          <li>点击跳转到对应分类页面</li>\r
          <li>支持拖拽排序（管理后台）</li>\r
          <li>悬停时显示更多信息（可选）</li>\r
          <li>配合骨架屏提升加载体验</li>\r
        </ul>\r
      </div>\r
    </DemoPage>\r
  );\r
}\r
`;export{n as default};
