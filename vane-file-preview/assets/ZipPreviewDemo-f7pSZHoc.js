...
\r
export default function ZipPreviewDemo() {\r
  const [selectedFile, setSelectedFile] = useState<FileInfo>(zipFiles[0]);\r
\r
  // 创建ZIP预览组件\r
...
\r
  return (\r
    <DemoPage\r
      title="ZIP预览"\r
      description="展示ZIP文件的预览功能，支持文件列表浏览、文件大小显示等功能"\r
    >\r
      <div\r
        style={{\r
          display: "flex",\r
      ...
      >\r
        ...
        >\r
          <h3 style={{ margin: "0 0 16px", fontSize: 16 }}>ZIP文件</h3>\r
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>\r
            {zipFiles.map((file, index) => (\r
              <button\r
                key={index}\r
              ...
              >\r
                ...
                >\r
                  {file.extension} • {(file.size / 1024 / 1024).toFixed(2)} MB\r
                </div>\r
              </button>\r
            ))}\r
          </div>\r
